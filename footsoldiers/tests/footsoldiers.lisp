...
(deftest get-players-input-for-turn
  (testing "provides the game as an alist to each of the players"
    (let* ((mp (alist-hash-table (list (cons (cons 4 3) *test-base1*)
           ...
           (gm (make-game :map mp :turns-remaining 10 :player1 player1 :player2 player2))
           (map-repr (list (cons (format-coord (cons 1 4)) 
                                 (list (cons "pos" (format-coord (cons 1 4)))
                                       (cons "health" 4)
                                       (cons "type" "SCOUT")
                                       (cons "team" "player1")
                                       (cons "destination" (format-coord (cons 5 5)))))
                           (cons (format-coord (cons 4 3)) (list (cons "team" "player1")))
                           (cons (format-coord (cons 5 4)) (list (cons "team" "player2")))))
           (player1-repr (list (cons "team" "player1")
                               (cons "money" 20)
                               (cons "base" (format-coord (cons 4 3)))
                               (cons "health" 25)))
           (player2-repr (list (cons "team" "player2")
                               (cons "money" 3)
                               (cons "base" (format-coord (cons 5 4)))
                               (cons "health" 7)))
